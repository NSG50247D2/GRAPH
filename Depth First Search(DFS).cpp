...
void DFS(int v,node **adj_list,int src,int visited[]){
    visited[src]=1;
    cout<<src<<" ";
    node *temp=adj_list[src];
    while(temp!=NULL){
        if(visited[temp->info]!=1){
            DFS(v,adj_list,temp->info,visited);
        }
        temp=temp->next;
    }
}
...
int main(){
    ...
    do{
        cout<<"Enter 1 for DFS, 2 for printing the graph, 3 for exit: ";
        cin>>ch;
        switch(ch)
        {
